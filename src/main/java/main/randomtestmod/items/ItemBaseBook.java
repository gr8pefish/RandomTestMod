...
public class ItemBaseBook extends ItemBase {
	...
	public ItemStack onItemRightClick(ItemStack itemStack, World world, EntityPlayer player) {
		switch (itemStack.getItemDamage()) {
			case 0: {
				player.openGui(RandomTestMod.instance, 0, world, (int) player.posX, (int) player.posY, (int) player.posZ);
				return itemStack;
			}
			case 1: {
				if (player.isSneaking() && !player.capabilities.isCreativeMode) {
					...
					player.addChatMessage(new ChatComponentTranslation("info.randomtestmod.chat.sneak"));
				}
			}
		}
		return itemStack;
	...
	public void addInformation(ItemStack itemStack, EntityPlayer player, List list, boolean par4) {
		if (itemStack.getItemDamage() == 1) {
			list.add(TextHelper.RED + TextHelper.localize("info.randomtestmod.tooltip.fire"));
		}
	}
}
